...
dfa_lagged <- read_csv(
   "data_cleaned/data02a_lagged-vdem.csv",
   col_types = cols(
      cowcode = col_factor(), 
      cpsidp = col_factor(),
      metfips = col_factor(),
...
fita_lagged_base <- brm(
   ...
   seed = 42,
   backend = "cmdstanr",
   file = "results/fita_lagged_base.RDS"
)

# View the results
summary(fita_lagged_base, prob = .9)

# Quick AME (ams using median here for a more robust result since I'm sampling)
point_interval(calc_ame(fita_lagged_base,
                        x_var = "lagged_vdem_corrupt_7079_z",
                        prop = .1)) # change to 1 for final analysis

# Predicted probabilities 
conditional_effects(fita_lagged_base, effects = "lagged_vdem_corrupt_7079_z")
...
fita_lagged_dem <- brm(
   ...
   seed = 42,
   backend = "cmdstanr",
   file = "results/fita_lagged_dem.RDS" # cache model (can be removed)
)

# View the results
summary(fita_lagged_dem, prob = .9)

# Quick AME (ams using median here for a more robust result since I'm sampling)
point_interval(calc_ame(fita_lagged_dem,
                        x_var = "lagged_vdem_corrupt_7079_z",
                        prop = .1)) # change to 1 for final analysis


# Predicted probabilities
conditional_effects(fita_lagged_dem,
...
fita_lagged_full <- brm(
   ...
   seed = 42,
   backend = "cmdstanr",
   file = "results/fita_lagged_full.RDS"
)

# View the results
summary(fita_lagged_full, prob = .9)

# Quick AME (ams using median here for a more robust result since I'm sampling)
point_interval(calc_ame(fita_lagged_full,
                        x_var = "lagged_vdem_corrupt_7079_z",
                        prop = .1)) # change to 1 for final analysis

# Predicted probabilities
conditional_effects(fita_lagged_full,
                    effects = "lagged_vdem_corrupt_7079_z",
                    prob = .9)
...
fita_lagged_dem_country <- brm(
   ...
   seed = 42,
   backend = "cmdstanr",
   file = "results/fita_lagged_dem_country.RDS"
)

# View the results
summary(fita_lagged_dem_country, prob = .95)

# Quick AME (ams using median here for a more robust result since I'm sampling)
point_interval(calc_ame(fita_lagged_dem_country,
                        x_var = "lagged_vdem_corrupt_7079_z",
                        prop = .1)) # change to 1 for final analysis
...
fita_lagged_full_country <- brm(
   ...
   seed = 42,
   backend = "cmdstanr",
   file = "results/fita_lagged_full_country.RDS" # cache model (can be removed)
)

# View the results
summary(fita_lagged_full_country, prob = .9)

# Quick AME (ams using median here for a more robust result since I'm sampling)
point_interval(calc_ame(fita_lagged_full_country,
                        x_var = "lagged_vdem_corrupt_7079_z",
                        prop = .1)) # change to 1 for final analysis

# Predicted probabilities
conditional_effects(fita_lagged_full_country,
                    effects = "lagged_vdem_corrupt_7079_z",
                    prob = .9)
# * Random effects -------------------------------------------

# Grab the random effects
rea_lagged_full_country <- 
   fita_lagged_full_country %>% 
   spread_draws(r_cowcode[condition, term]) %>% 
   mean_qi() %>% 
   mutate(sig = ifelse(.lower > 0 | .upper < 0, "sig", "insig"))

# Make the plot
plot_randoms(rea_lagged_full_country)  +
   theme(axis.text.x = element_text(size = 4, angle = 45),
         panel.grid.major.x = element_blank())
